...
static __global__ void mul_mat_vec_f(
        const T * __restrict__ x, const float * __restrict__ y, const int32_t * __restrict__ ids, const lm_ggml_cuda_mm_fusion_args_device fusion, float * __restrict__ dst,
    ...
    lm_ggml_glu_op glu_op = lm_ggml_glu_op::LM_GGML_GLU_OP_SWIGLU;
...
#pragma unroll
            for (int j = 0; j < ncols_dst; ++j) {
                ...
                lm_ggml_cuda_mad(sumf[j], tmpx.x, tmpy.x);
                lm_ggml_cuda_mad(sumf[j], tmpx.y, tmpy.y);

                if constexpr (has_fusion) {
                    if (use_gate) {
                        lm_ggml_cuda_mad(sumf_gate[j], tmpx_gate.x, tmpy.x);
                        lm_ggml_cuda_mad(sumf_gate[j], tmpx_gate.y, tmpy.y);
                    }
                }
            }
        }
...
//TODO: add support for lm_ggml_cuda_mad for hip_bfloat162
...
#pragma unroll
            for (int j = 0; j < ncols_dst; ++j) {
                const float2 tmpy = y2[j*stride_col_y2 + col2];
                const float tmpx0 = lm_ggml_cuda_cast<float>(reinterpret_cast<const nv_bfloat16 *>(&tmpx)[0]);
                const float tmpx1 = lm_ggml_cuda_cast<float>(reinterpret_cast<const nv_bfloat16 *>(&tmpx)[1]);
                lm_ggml_cuda_mad(sumf[j], tmpx0, tmpy.x);
                lm_ggml_cuda_mad(sumf[j], tmpx1, tmpy.y);

                if constexpr (has_fusion) {
                    if (use_gate) {
                        const float tmpx0_gate = lm_ggml_cuda_cast<float>(reinterpret_cast<const nv_bfloat16 *>(&tmpx_gate)[0]);
                        const float tmpx1_gate = lm_ggml_cuda_cast<float>(reinterpret_cast<const nv_bfloat16 *>(&tmpx_gate)[1]);
                        lm_ggml_cuda_mad(sumf_gate[j], tmpx0_gate, tmpy.x);
                        lm_ggml_cuda_mad(sumf_gate[j], tmpx1_gate, tmpy.y);
                    }
                }
            }
        }
...
#pragma unroll
        ...
        if (use_gate) {
            ...
            switch (glu_op) {
                case LM_GGML_GLU_OP_SWIGLU:
                    value *= lm_ggml_cuda_op_silu_single(gate_value);
                    break;
                case LM_GGML_GLU_OP_GEGLU:
                    value *= lm_ggml_cuda_op_gelu_single(gate_value);
                    break;
                case LM_GGML_GLU_OP_SWIGLU_OAI: {
                    value = lm_ggml_cuda_op_swiglu_oai_single(gate_value, value);
                    break;
                }
                default:
                    break;
    ...
    if constexpr (!has_fusion) {
        LM_GGML_UNUSED_VARS(use_gate, use_bias, use_gate_bias, glu_op, gate_x, x_bias, gate_bias, sumf_gate);
    }
}

template<typename T, typename type_acc, int ncols_dst, int block_size>
static void mul_mat_vec_f_switch_fusion(
        const T * x, const float * y, const int32_t * ids, const lm_ggml_cuda_mm_fusion_args_device fusion, float * dst,
...
void launch_mul_mat_vec_f_cuda(
    ...
    if(lm_ggml_cuda_info().devices[device].cc > LM_GGML_CUDA_CC_OFFSET_AMD && lm_ggml_cuda_info().devices[device].cc < LM_GGML_CUDA_CC_RDNA1) {
        max_block_size = 128;
    }
...
static void mul_mat_vec_f_cuda(
        ...
        const int64_t stride_channel_x, const int64_t stride_channel_y, const int64_t stride_channel_dst, const int64_t nsamples_x,
        const int64_t nsamples_dst, const int64_t stride_sample_x, const int64_t stride_sample_y, const int64_t stride_sample_dst,
        enum lm_ggml_prec prec, cudaStream_t stream) {

    if constexpr(std::is_same_v<T, half>) {
        if (prec == LM_GGML_PREC_DEFAULT) {
...
void lm_ggml_cuda_mul_mat_vec_f(lm_ggml_backend_cuda_context & ctx, const lm_ggml_tensor * src0, const lm_ggml_tensor * src1, const lm_ggml_tensor * ids, lm_ggml_tensor * dst,
    ...
    const enum lm_ggml_prec prec = fast_fp16_available(cc) ? lm_ggml_prec(dst->op_params[0]) : LM_GGML_PREC_F32;
...
void lm_ggml_cuda_op_mul_mat_vec_f(
    ...
    // lm_ggml_cuda_op provides single, contiguous matrices
    ...
    lm_ggml_cuda_mm_fusion_args_device empty{};
    switch (src0->type) {
        case LM_GGML_TYPE_F32: {
    ...
    LM_GGML_UNUSED_VARS(ctx, src1, dst, src1_ddq_i, src1_ncols, src1_padded_row_size);
}
...
bool lm_ggml_cuda_should_use_mmvf(enum lm_ggml_type type, int cc, const int64_t * src0_ne, const size_t * src0_nb, int64_t ne11) {
    ...
    switch (type) {
        case LM_GGML_TYPE_F32:
            if (LM_GGML_CUDA_CC_IS_NVIDIA(cc)) {
                ...
                return ne11 <= 3;
            } else if (LM_GGML_CUDA_CC_IS_AMD(cc)) {
        ...
        case LM_GGML_TYPE_F16:
            if (LM_GGML_CUDA_CC_IS_NVIDIA(cc)) {
                ...
                if (fp16_mma_hardware_available(cc)) {
                    if (LM_GGML_CUDA_CC_IS_RDNA3(cc)) {
                        return ne11 <= 3;
                    }
                    if (LM_GGML_CUDA_CC_IS_RDNA4(cc)) {
                        return ne11 <= 5;
                    }
                    return ne11 <= 2;
                }
